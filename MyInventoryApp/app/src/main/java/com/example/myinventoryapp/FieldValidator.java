...
public class FieldValidator {
    ...
    /**
     * Checks a barcode to ensure it was scanned properly
     * @param value the value returned by scanning the barcode
     * @return boolean
     */
    public static boolean validateBarcode(String value) {
        // The first 6 digits of a barcode are the prefix, the prefix identifies brand owners
        // The next 5 digits are assigned by the retailer to reference an item, thus  called "item reference"
        // The last digit is the check digit, it is generated by a formula to check the barcode was read right
        int length = value.length();
        if (length != 12 && length != 13) {
            return false;
        }

        // Separate the value into even, odd and check
        int check = 0;
        if (length == 12) {
            check = Character.getNumericValue(value.charAt(11));
        } else {
            check = Character.getNumericValue(value.charAt(12));
        }
        int even = 0;
        int odd = 0;
        ...
        // calculate and check against the check digit
        if (length == 12) {
            odd = odd * 3;
        } else {
            even = even * 3;
